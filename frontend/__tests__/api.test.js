...
describe('API Service — getUsuarios', () => {
    it('retorna la lista de usuarios cuando el servidor responde OK', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('http://localhost:3001/api/usuarios', { credentials: 'include' });
    });
...
describe('API Service — getUsuario', () => {
    it('obtiene un usuario por ID', async () => {
        ...
        expect(result).toEqual(mockData);
        expect(global.fetch).toHaveBeenCalledWith('http://localhost:3001/api/usuarios/2', { credentials: 'include' });
    });
